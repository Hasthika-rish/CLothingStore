import { db, collection, getDocs, addDoc, doc, setDoc, query, where } from './firebase-config.js';
  ...
  try {
    ...
    if (categoriesCheck.empty) {
      ...
      const sampleCategories = [
        { id: "cat_women", name: "Women", gender: "Women", parent: "", image: "images/women_new.png" },
        { id: "cat_men", name: "Men", gender: "Men", parent: "", image: "images/men_new.png" },
        { id: "cat_other", name: "Other", gender: "Unisex", parent: "", image: "https://images.unsplash.com/photo-1523293182086-7651a899d37f?w=400" },
        { id: "cat_kids", name: "Kids Section", gender: "Kids", parent: "", image: "https://images.unsplash.com/photo-1519238263530-99bdd11df2ea?w=400" }
      ];
    ...
    // 9. Seed Staff
    const sarahQuery = await getDocs(query(collection(db, "staff"), where("email", "==", "sarah@anjiana.com")));
    if (sarahQuery.empty) {
      console.log("Seeding Sarah Connor manager account...");
      await addDoc(collection(db, "staff"), { name: "Sarah Connor", email: "sarah@anjiana.com", role: "Manager", status: "Approved", createdAt: new Date() });
    }
    const adminQuery = await getDocs(query(collection(db, "staff"), where("email", "==", "admin@sageanjiana.com")));
    if (adminQuery.empty) {
      console.log("Seeding Admin root account...");
      await addDoc(collection(db, "staff"), { name: "Administrator Root", email: "admin@sageanjiana.com", role: "Admin", status: "Approved", createdAt: new Date() });
    }

    // 10. Seed Bank Deposit verifying order and COD pending order for testing if they don't exist
    const bankCheck = await getDocs(query(collection(db, "orders"), where("paymentMethod", "==", "BANK_DEPOSIT")));
    if (bankCheck.empty) {
      console.log("Seeding test Bank Deposit and COD orders...");
      // Find a seeded product first to link to the order items
      const prodSnap = await getDocs(collection(db, "products"));
      let testProd = { id: "test_product", name: "Premium Oxford Cotton Shirt", price: 85.00, imageUrl: "https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=400" };
      if (!prodSnap.empty) {
        const firstProd = prodSnap.docs[0];
        testProd = { id: firstProd.id, name: firstProd.data().name, price: firstProd.data().discount || firstProd.data().price, imageUrl: firstProd.data().imageUrl };
      }

      await addDoc(collection(db, "orders"), {
        customer: { firstName: "Sarah", lastName: "Connor", email: "sarah@sky.net", phone: "+1 (555) 019-9283", address: "742 Evergreen Terrace", city: "Los Angeles", postalCode: "90001" },
        items: [
          { id: testProd.id, name: testProd.name, price: testProd.price, size: "S", color: "Sage Green", quantity: 1, imageUrl: testProd.imageUrl }
        ],
        subtotal: testProd.price,
        shipping: 10.00,
        totalAmount: testProd.price + 10.00,
        paymentStatus: "VERIFYING",
        paymentMethod: "BANK_DEPOSIT",
        paymentProofUrl: "https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?w=500",
        orderStatus: "Pending",
        createdAt: new Date()
      });

      await addDoc(collection(db, "orders"), {
        customer: { firstName: "Bruce", lastName: "Wayne", email: "bruce@waynecorp.com", phone: "+1 (555) 911-3000", address: "1007 Mountain Drive", city: "Gotham", postalCode: "53540" },
        items: [
          { id: testProd.id, name: testProd.name, price: testProd.price, size: "M", color: "Sage Green", quantity: 1, imageUrl: testProd.imageUrl }
        ],
        subtotal: testProd.price,
        shipping: 10.00,
        totalAmount: testProd.price + 10.00,
